...
async function disableMessage(message) {
    ...
    try {
        const rows = message.components.map(row => ({
            type: 1, components: row.components.map(component => ({
                type: 2, custom_id: component.customId, style: component.style, label: component.label || undefined, emoji: component.emoji ? {
                    id: component.emoji.id || undefined, name: component.emoji.name || undefined
                }
...
function bind(player, guild) {
    ...
    player.onEvent = async(type, ...args) => {
        console.log(`[${guild.id}] ${type}${args.length ? ` $ {
            args.map(x => typeof x === 'object' ? x.title || x.id: x).join(' ')
        }
        ` : ''}`);
