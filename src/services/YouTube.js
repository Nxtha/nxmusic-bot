...
function spawnYtdlp(url, flags = {}, options = {}) {
...
async function info(url) {
  await rt();
  const out = await execText(url, addCommon({ dumpSingleJson: true, skipDownload: true, noPlaylist: true }));
  try { return JSON.parse(out); } catch { throw new Error('yt-dlp returned invalid metadata'); }
}
...
async function resolve(url) {
  await rt();
  const out = await execText(url, addCommon({ dumpSingleJson: true, noPlaylist: true, skipDownload: true }));
  let parsed;
  try { parsed = JSON.parse(out); } catch { throw new Error('NO_RESULTS'); }
...
async function getTracks(query, limit = 100) {
  ...
  if (/^https?:\/\//i.test(input)) {
    ...
    try { return [await resolve(input)]; } catch { return []; }
  }
...
async function stream(track, offset = 0) {
  ...
  ff.on('error', () => {});
  ff.stdin.on('error', err => { if (err?.code !== 'EPIPE') fferr += ` ${err.message || err}`; });
  ff.stdout.on('error', () => {});
  ...
  const cleanup = () => {
    ...
    cleanupDone = true;
    try { y.stdout?.unpipe(ff.stdin); } catch {}
  ...
  done.catch(() => { if (!destroyed) cleanup(); });
  return { stream: output, done, destroy };
}

module.exports = { init: rt, info, search, resolve, playlist, getTracks, stream };
