const {createAudioPlayer,createAudioResource,joinVoiceChannel,AudioPlayerStatus,VoiceConnectionStatus,entersState,NoSubscriberBehavior,StreamType}=require('@discordjs/voice');
const Queue=require('./Queue');
const YouTube=require('../services/YouTube');
const {randomMusicQuery,queryForGenre,cleanCandidates}=require('./Autoplay');
...
class Player{
  ...
  async connect(channel){
    ...
    if(!this.connection||this.connection.state.status===VoiceConnectionStatus.Destroyed){
      ...
      this.voiceChannelId=channel.id;this.connection.subscribe(this.player);this.connection.on(VoiceConnectionStatus.Disconnected,()=>this._reconnect());
    }
  ...
  async _reconnect(){
    ...
    try{await Promise.race([entersState(this.connection,VoiceConnectionStatus.Signalling,5000),entersState(this.connection,VoiceConnectionStatus.Connecting,5000)]);return}catch{}
    try{this.connection.destroy()}catch{}
    try{this.connection=joinVoiceChannel({channelId:this.channel.id,guildId:this.guildId,adapterCreator:this.channel.guild.voiceAdapterCreator,selfDeaf:true});this.connection.subscribe(this.player);this.onEvent('reconnect')}catch(e){this.onEvent('error',e)}
  }

  async enqueue(tracks){
    this.endedAt=0;if(this.idleTimer){clearTimeout(this.idleTimer);this.idleTimer=null;}
    this.queue.addMany(tracks);this.onEvent('queue-add',tracks.length);if(!this.current)await this._advance();
  }

  async _start(track,offset=0){
    const requester=track.requester||null;this.current={...track,requester};
  ...
  async _recoverPlayback(error){
    ...
    try{
      ...
      if(this.recoveryAttempts<3){this.recovering=false;return this._recoverPlayback(e)}
      this.recoveryAttempts=0;this.recovering=false;this.skipRequested=true;this.player.stop(true);return;
    }finally{
  ...
  _scheduleIdleDisconnect(){
    ...
    this.idleTimer=setTimeout(()=>{
      ...
      try{this.connection?.destroy()}catch{}
      this.connection=null;this.voiceChannelId=null;this.channel=null;
  ...
  async _advance(){
    if(this.busy||this.recovering)return;this.busy=true;
    try{
      for(let attempt=0;attempt<6;attempt++){
        if(this.loop==='song'&&this.current&&!this.skipRequested){try{await this._start(this.current);return}catch(e){this.onEvent('error',e);continue}}
        ...
        try{await this._start(next);return}catch(e){this.onEvent('error',e);this.current=previous}
      }
      this.current=null;this.endedAt=Date.now();this.sessionChannelId=null;this.onEvent('idle');
      this._scheduleIdleDisconnect()
    }catch(e){this.onEvent('error',e)}finally{this.busy=false}
  }

  pause(){this.player.pause();this.pausedAt=this.elapsed();this.onEvent('pause')}
  ...
  elapsed(){if(!this.current)return 0;return this.paused?this.pausedAt:Math.max(0,(Date.now()-this.startedAt)/1000)}
  get paused(){return this.player.state.status===AudioPlayerStatus.Paused}
}
