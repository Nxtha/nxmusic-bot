...
const {get}=require('../services/PlayerManager');
const Activity=require('../services/ActivityManager');
const {messageFor}=require('../utils/errors');
...
module.exports=async interaction=>{
  try{
    ...
    if(interaction.isStringSelectMenu()&&interaction.customId.startsWith('nx:autoplay-genre:')){
      ...
      await interaction.update({content:'',...actionCard('🤖','Autoplay Updated',`Mode: **Fixed Genre • ${genre}** by <@${interaction.user.id}>.`)});
    ...
    if(interaction.isStringSelectMenu()&&interaction.customId.startsWith('nx:search:')){
      const key=interaction.customId.slice(10),xs=Search.cache.get(key);
      if(!xs)throw new Error('Search expired.');
      const idx=Number(interaction.values[0]),t=xs[idx];
      ...
      if(p.voiceChannelId&&p.voiceChannelId!==ch.id)throw new Error('DIFFERENT_VC');
      await p.connect(ch);
      if(!p.sessionChannelId){p.sessionChannelId=interaction.channelId;Activity.setChannel(p,interaction.channel);}
    ...
    if(interaction.isButton()&&interaction.customId.startsWith('nx:')){
      ...
      if(!Activity.isActiveMessage(p,interaction.message)){
        await interaction.reply({content:'⚠️ This Music Activity is no longer active.',ephemeral:true});
        return;
      }
    ...
    if(interaction.isModalSubmit()&&interaction.customId==='nx:volume-modal'){
      ...
      p.setVolume(v);
      await interaction.reply({...actionCard('🔊','Volume Changed',`Volume set to **${v}%** by <@${interaction.user.id}>.`),ephemeral:true});
      setTimeout(()=>interaction.deleteReply().catch(()=>{}),10000);
    }
  }catch(e){
    const msg=messageFor(e);
    if(interaction.deferred)await interaction.editReply({content:msg,components:[]}).catch(()=>{});
    else if(interaction.replied)await interaction.followUp({content:msg,ephemeral:true}).catch(()=>{});
