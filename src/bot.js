...
const voiceState = require('./events/voiceStateUpdate');
...
client.once('ready', async c => {
    ...
    try {
        const body = commands.map(x => x.data.toJSON());
        if (config.guildId) await rest.put(Routes.applicationGuildCommands(config.clientId, config.guildId), {
            body
        });
        else await rest.put(Routes.applicationCommands(config.clientId), {
            body
        });
